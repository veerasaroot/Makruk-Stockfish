...
void FeatureTransformer<TransformedFeatureDimensions, accPtr>::
  apply_accumulator_updates_incremental(StateInfo*            computed_st,
    ...
    {
        const auto accIn =
          reinterpret_cast<const int16x8_t*>(&(st->*accPtr).accumulation[Perspective][0]);
        const auto accOut = reinterpret_cast<int16x8_t*>(
        ...
        if (removed[0].size() == 1)
        {
            for (IndexType k = 0; k < HalfDimensions * sizeof(WeightType) / 16; ++k)
                accOut[k] = vaddq_s16(vsubq_s16(accIn[k], columnR0[k]), columnA[k]);
        }
        else
        {
            ...
            for (IndexType k = 0; k < HalfDimensions * sizeof(WeightType) / 16; ++k)
                accOut[k] =
                  vsubq_s16(vaddq_s16(accIn[k], columnA[k]), vaddq_s16(columnR0[k], columnR1[k]));
        }

        const auto accPsqtIn =
          reinterpret_cast<const int32x4_t*>(&(st->*accPtr).psqtAccumulation[Perspective][0]);
        const auto accPsqtOut = reinterpret_cast<int32x4_t*>(
        ...
        if (removed[0].size() == 1)
        {
            for (IndexType k = 0; k < PSQTBuckets * sizeof(PSQTWeightType) / 16; ++k)
                accPsqtOut[k] = vaddq_s32(vsubq_s32(accPsqtIn[k], columnPsqtR0[k]), columnPsqtA[k]);
        }
        else
        {
            ...
            for (IndexType k = 0; k < PSQTBuckets * sizeof(PSQTWeightType) / 16; ++k)
                accPsqtOut[k] = vsubq_s32(vaddq_s32(accPsqtIn[k], columnPsqtA[k]),
                                          vaddq_s32(columnPsqtR0[k], columnPsqtR1[k]));
        }
    }
    else
    {
        ...
        for (IndexType j = 0; j < HalfDimensions / Details::TileHeight; ++j)
        {
            ...
            const auto accTileIn = reinterpret_cast<const int16x8_t*>(
              &(st->*accPtr).accumulation[Perspective][offsetRow]);
            ...
            for (IndexType i = 0; i < N; ++i)
            {
                ...
                const auto accTileOut = reinterpret_cast<int16x8_t*>(
