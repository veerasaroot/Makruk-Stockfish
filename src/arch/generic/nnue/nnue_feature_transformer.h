...
void FeatureTransformer<TransformedFeatureDimensions, accPtr>::
  apply_accumulator_updates_incremental(StateInfo*            computed_st,
    ...
    for (IndexType i = 0; i < N; ++i)
    {
        ...
        std::memcpy((states_to_update[i]->*accPtr).psqtAccumulation[Perspective],
        ...
        for (const auto index : removed[i])
        {
            ...
            for (IndexType j = 0; j < HalfDimensions; ++j)
                (st->*accPtr).accumulation[Perspective][j] -= weights[wOffset + j];

            for (IndexType k = 0; k < PSQTBuckets; ++k)
                (st->*accPtr).psqtAccumulation[Perspective][k] -= psqtWeights[pwOffset + k];
        }
