    ...
    if (N == 1 && (removed[0].size() == 1 || removed[0].size() == 2) && added[0].size() == 1)
    {
        ...
        if (removed[0].size() == 1)
        {
            for (IndexType k = 0; k < HalfDimensions * sizeof(WeightType) / sizeof(vec_t); ++k)
                accOut[k] = _mm_add_epi16_v(_mm_sub_epi16_v(accIn[k], columnR0[k]), columnA[k]);
        }
        else
        {
            ...
            for (IndexType k = 0; k < HalfDimensions * sizeof(WeightType) / sizeof(vec_t); ++k)
                accOut[k] = _mm_sub_epi16_v(_mm_add_epi16_v(accIn[k], columnA[k]),
                                            _mm_add_epi16_v(columnR0[k], columnR1[k]));
        }
        ...
        if (removed[0].size() == 1)
        {
            for (IndexType k = 0; k < PSQTBuckets * sizeof(PSQTWeightType) / sizeof(psqt_vec_t);
                 ++k)
                accPsqtOut[k] =
                  _mm_add_epi32_v(_mm_sub_epi32_v(accPsqtIn[k], columnPsqtR0[k]), columnPsqtA[k]);
        }
        else
        {
            ...
            for (IndexType k = 0; k < PSQTBuckets * sizeof(PSQTWeightType) / sizeof(psqt_vec_t);
                 ++k)
                accPsqtOut[k] = _mm_sub_epi32_v(_mm_add_epi32_v(accPsqtIn[k], columnPsqtA[k]),
                                                _mm_add_epi32_v(columnPsqtR0[k], columnPsqtR1[k]));
        }
    }
    else
    {
        ...
        for (IndexType j = 0; j < HalfDimensions / Details::TileHeight; ++j)
        {
            ...
            for (IndexType i = 0; i < N; ++i)
            {
                ...
                const auto accTileOut = reinterpret_cast<vec_t*>(
