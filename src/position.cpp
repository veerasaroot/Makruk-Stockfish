...
constexpr std::string_view PieceToChar(" PMSNRK  pmsnrk");
...
void Position::set_check_info() const {
    ...
    st->checkSquares[QUEEN]  = attacks_bb<QUEEN>(ksq);
...
void Position::update_slider_blockers(Color c) const {
    ...
    Bitboard snipers = attacks_bb<ROOK>(ksq)
                     & pieces(~c);
...
bool Position::pseudo_legal(const Move m) const {
    ...
    assert(m.promotion_type() - QUEEN != NO_PIECE_TYPE);
    ...
    if (type_of(pc) == PAWN)
    {
        // We have already handled promotion moves, so destination cannot be on the 6th/3rd rank
        ...
        if (!(pawn_attacks_bb(us, from) & pieces(~us) & to)  // Not a capture
            ...
            && !((from + 2 * pawn_push(us) == to)            // Not a double push
                 && (relative_rank(us, from) == RANK_4) && empty(to) && empty(to - pawn_push(us))))
            return false;
    }
